...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private const val REQUEST_PERMISSION_CODE =
            125 //integer constant used when requesting app permissions
    }
    ...
    private val isRegistrationInProgress =
        AtomicBoolean(false) //This boolean can be accessed and updated by multiple threads
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mHandler =
            Handler(Looper.getMainLooper()) //initiates a Handler that uses the app's main looper (runs on the UI thread)
    }
    ...
    private fun startSDKRegistration() {
        GlobalScope.launch {
             if (isRegistrationInProgress.compareAndSet(
                false,
                true
            )
            ) { //if isRegistrationInProgress is false, set to true
    ...
    private fun notifyStatusChange() {
        ...
        mHandler.postDelayed(
            updateRunnable,
            500
        ) //adds a new updateRunnable to the message queue, which is executed 0.5 seconds after
    }
    ...
    private val updateRunnable: Runnable = Runnable {
        val intent =
            Intent(FLAG_CONNECTION_CHANGE) //This intent lets the broadcast receiver (itself) know that a connection change has occurred.
